...
/// <summary>
/// Minimal 8×8 bitmap font for ASCII printable characters (0x20–0x7E).
/// Each character is 8 bytes; bit 7 of each byte is the leftmost pixel.
/// Based on the classic IBM CP437 / VGA 8x8 bitmap font (public domain).
/// </summary>
internal static class BitmapFont
{
    ...
    // Rows top-to-bottom, MSB = leftmost pixel.
    ...
    /// <summary>
    /// Retrieves the 8 row-bytes for the given ASCII character.
    ...
    public static byte[] GetGlyph(char c)
    {
        int idx = c - 0x20;
        if (idx < 0 || idx >= 95)
            return new byte[8]; // blank

        var glyph = new byte[8];
        System.Array.Copy(Data, idx * 8, glyph, 0, 8);
        return glyph;
    }
}
